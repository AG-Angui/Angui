...
本仓库目前处于 **MVP 纵向闭环开发阶段**。React 工作台、Rust/Actix Web 后端、SeaORM 数据层、可撤销会话认证、案件级角色授权、版本化迁移，以及案件创建、查询、成员授权、线索提交、人工审核和状态流转已经落地。受控 AI 草稿、地图和任务仍在持续完善；密码找回、多因素认证和生产部署属于后续实现范围。当前实现不能视为已经部署或经过真实搜救验证的产品。
...
- 访问申请：统一登录入口保留邮箱密码认证；新用户经邮件链接验证、管理员人工审核并通过一次性链接设置初始密码。消息投递以 SMTP Email 为首个通道，后续可复用投递记录和模板事件接入企业微信、钉钉等通道。
- AI：通义千问/百炼 Agent 与 RAG，用于辅助问询、结构化、摘要、知识问答和案例整理。
- 地图：高德地图 JS API/Web 服务 API，用于点位、POI、路线和轨迹展示。

React、Rust/Actix Web、SeaORM 数据持久化和首批身份权限已经进入仓库。当前只在 SQLite 上完成实际迁移与认证/RBAC HTTP 联调；PostgreSQL/MySQL 已提供驱动、方言 SQL 和 CI 服务容器验证配置。AI 已实现受控问询、家属初审、线索草稿、摘要草稿和案例整理草稿，但必须经过人工确认和失败降级，不能被描述为模型自动确认事实或可用于真实搜救。地图、任务和生产级账号生命周期仍在持续完善。
...
当前实现的 AI 能力仅生成受控草稿：家属问询的可跳过追问、线索字段候选、案件摘要草稿与案例整理候选。它们都必须经过权限检查、审计和人工审核；AI 永远不会自动确认事实、发布案情、派发任务或将案例写入知识库。Gateway 的单次执行最多发起两次 HTTP 调用，且 JSON 修复与合规 Provider 切换互斥；未配置或不合规的模型、超时和无效输出都会回退到静态问询或手工编辑流程。
