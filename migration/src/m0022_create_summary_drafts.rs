...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ensure_rollback_is_safe(
            manager,
            &[(
                "summary drafts exist",
                "SELECT 1 FROM summary_drafts LIMIT 1",
            )],
        )
        .await?;
        execute_script(
            ...
            sql_for_backend(
                manager,
                include_str!("../sql/sqlite/down/0022_drop_summary_drafts.sql"),
                include_str!("../sql/postgres/down/0022_drop_summary_drafts.sql"),
                include_str!("../sql/mysql/down/0022_drop_summary_drafts.sql"),
            ),
        )
        .await
    }
