...
-- statement-break
CREATE TABLE users_without_learner_role (
    id TEXT PRIMARY KEY,
...
-- statement-break
INSERT INTO users_without_learner_role (id, email, display_name, role, password_hash, status, created_at, updated_at)
SELECT id, email, display_name, role, password_hash, status, created_at, updated_at FROM users;
-- statement-break
DROP TABLE users;
-- statement-break
ALTER TABLE users_without_learner_role RENAME TO users;
-- statement-break
