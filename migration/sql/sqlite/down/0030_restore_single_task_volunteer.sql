...
CREATE TABLE task_location_reports_single (
    ...
    FOREIGN KEY (task_id) REFERENCES tasks(id) ON DELETE CASCADE,
    FOREIGN KEY (task_id, volunteer_user_id) REFERENCES task_assignments_single(task_id, volunteer_user_id) ON DELETE CASCADE
);
INSERT INTO task_location_reports_single SELECT report.id, report.task_id, report.volunteer_user_id, report.source, report.latitude, report.longitude, report.accuracy_meters, report.captured_at, report.retention_expires_at, report.created_at FROM task_location_reports report JOIN task_assignments_single assignment ON assignment.task_id = report.task_id AND assignment.volunteer_user_id = report.volunteer_user_id;
...
ALTER TABLE task_location_reports_single RENAME TO task_location_reports;
