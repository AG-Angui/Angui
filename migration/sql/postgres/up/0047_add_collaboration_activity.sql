...
-- statement-break
CREATE TABLE voice_reports (id TEXT PRIMARY KEY, space_id TEXT NOT NULL REFERENCES collaboration_spaces(id) ON DELETE CASCADE, case_id TEXT NOT NULL REFERENCES cases(id) ON DELETE CASCADE, reporter_id TEXT NOT NULL REFERENCES users(id), object_key TEXT NOT NULL UNIQUE, content_type TEXT NOT NULL, byte_size BIGINT NOT NULL CHECK (byte_size > 0), status TEXT NOT NULL CHECK (status IN ('uploaded','transcribing','transcribed','draft_ready','failed','reviewed')), created_at TEXT NOT NULL, failed_reason TEXT NULL);
-- statement-break
