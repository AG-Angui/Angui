CREATE TABLE ai_prompt_templates (
    id VARCHAR(36) PRIMARY KEY, purpose VARCHAR(32) NOT NULL CHECK (purpose IN ('intake_next_question', 'intake_profile_draft', 'clue_draft', 'case_summary_draft', 'knowledge_answer', 'case_archive_draft')), version VARCHAR(128) NOT NULL, system_instruction TEXT NOT NULL,
    status VARCHAR(16) NOT NULL CHECK (status IN ('draft', 'published', 'retired')), created_by_user_id VARCHAR(36), published_by_user_id VARCHAR(36), published_at VARCHAR(40), created_at VARCHAR(40) NOT NULL, updated_at VARCHAR(40) NOT NULL,
