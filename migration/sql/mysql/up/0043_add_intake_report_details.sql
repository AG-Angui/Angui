CREATE TABLE intake_session_photos (
    ...
    CONSTRAINT chk_intake_photo_content_type CHECK (content_type IN ('image/jpeg', 'image/png')),
    CONSTRAINT chk_intake_photo_size CHECK (byte_size > 0),
    CONSTRAINT fk_intake_photo_session FOREIGN KEY (session_id) REFERENCES intake_sessions(id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_intake_photo_creator FOREIGN KEY (created_by_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE RESTRICT
);
-- statement-break
CREATE INDEX idx_intake_session_photos_session ON intake_session_photos(session_id, created_at);
-- statement-break
CREATE TABLE intake_question_definition_status_backup_m0043 (
    question_id VARCHAR(36) PRIMARY KEY,
    previous_status VARCHAR(16) NOT NULL,
    CONSTRAINT chk_intake_question_definition_status_backup_m0043 CHECK (previous_status = 'active'),
    CONSTRAINT fk_intake_question_definition_status_backup_m0043_question FOREIGN KEY (question_id) REFERENCES intake_question_definitions(id) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB;
-- statement-break
