    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-7 sm:px-6 lg:px-10 lg:py-10">
      ...
      <section className="mb-7 border-y border-slate-200 bg-white" aria-labelledby="knowledge-search-title">
        ...
        <div className="grid gap-2 p-5 sm:grid-cols-[1fr_10rem_10rem_auto_auto]">
          ...
          <select aria-label="知识库分类" className="h-10 rounded-md border border-slate-300 bg-white px-3" value={knowledgeCategory} onChange={(event) => setKnowledgeCategory(event.target.value)}><option value="">全部分类</option>{knowledgeCategories.map((category) => <option key={category} value={category}>{category}</option>)}</select>
          <select aria-label="知识库标签" className="h-10 rounded-md border border-slate-300 bg-white px-3" value={knowledgeTag} onChange={(event) => setKnowledgeTag(event.target.value)}><option value="">全部标签</option>{knowledgeTags.map((tag) => <option key={tag} value={tag}>{tag}</option>)}</select>
