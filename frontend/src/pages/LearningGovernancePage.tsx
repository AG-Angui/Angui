...
function LifecycleActions({
  ...
  const canAbandon = canAbandonCorrection && lifecycle.state !== "published" && lifecycle.state !== "withdrawn";
  return action || canAbandon ? <div className="grid justify-items-end gap-1">{action && <Button size="sm" variant={action === "withdraw" ? "danger" : "secondary"} isDisabled={busy || disabledForIndependence} onPress={() => onAction(action)}>{labels[action]}</Button>}{canAbandon && <Button size="sm" variant="danger" isDisabled={busy} onPress={() => onAction("withdraw")}>放弃更正</Button>}{description && <span className="max-w-64 text-right text-xs leading-5 text-amber-700">{description}</span>}</div> : null;
}
...
export function LearningGovernancePage() {
  ...
  const submitQuestion = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    void createManagedLearningQuestion(token, { ...questionForm, tags: parseTags(questionTags), options }).then(() => { setQuestionForm({ ...questionForm, prompt: "", tags: [], options: [], correct_option_id: "", explanation: "", previous_version_id: null, submission_reason: "" }); setQuestionTags(""); setOptionLines(""); return load(); }).catch((cause) => setOperationError(errorMessage(cause))).finally(() => setBusyId(""));
  };
  ...
  return <main className="mx-auto w-full max-w-7xl px-4 py-7 sm:px-6 lg:px-10 lg:py-10">
    ...
    <section className="border-y border-slate-200 bg-white"><header className="flex items-center gap-2 border-b border-slate-200 px-5 py-4"><FilePlus2 size={18} aria-hidden="true" /><h2 className="m-0 text-base font-bold text-slate-950">新增学习资源</h2></header><form className="grid gap-4 p-5 lg:grid-cols-2" onSubmit={submitResource}>
      ...
      <label className="grid gap-1 text-sm text-slate-700">资源类型<select className="h-10 rounded-md border border-slate-300 bg-white px-3" value={resourceForm.resource_type} onChange={(event) => setResourceForm({ ...resourceForm, resource_type: event.target.value as CreateLearningResourceInput["resource_type"] })}><option value="manual">手册</option><option value="prevention">防走失知识</option><option value="team_intro">队伍介绍</option><option value="case_study">案例学习</option></select></label><label className="grid gap-1 text-sm text-slate-700">更正上一版本<select className="h-10 rounded-md border border-slate-300 bg-white px-3" value={resourceForm.previous_version_id ?? ""} onChange={(event) => setResourceForm({ ...resourceForm, previous_version_id: event.target.value || null })}><option value="">新增首版</option>{resources.filter((resource) => resource.lifecycle.state === "published").map((resource) => <option key={resource.id} value={resource.id}>{resource.title} v{resource.version}</option>)}</select></label><label className="grid gap-1 text-sm text-slate-700">可见范围<select className="h-10 rounded-md border border-slate-300 bg-white px-3" value={resourceForm.visibility} onChange={(event) => setResourceForm({ ...resourceForm, visibility: event.target.value as CreateLearningResourceInput["visibility"] })}><option value="learner">新人</option><option value="volunteer">志愿者</option><option value="authenticated">登录用户</option><option value="public">公开</option></select></label>
    ...
    <section className="mt-7 border-y border-slate-200 bg-white"><header className="flex items-center gap-2 border-b border-slate-200 px-5 py-4"><BookCheck size={18} aria-hidden="true" /><h2 className="m-0 text-base font-bold text-slate-950">新增学习题目</h2></header><form className="grid gap-4 p-5 lg:grid-cols-2" onSubmit={submitQuestion}>
      <label className="grid gap-1 text-sm text-slate-700">来源资源<select className="h-10 rounded-md border border-slate-300 bg-white px-3" value={questionForm.source_resource_id} onChange={(event) => setQuestionForm({ ...questionForm, source_resource_id: event.target.value })} required><option value="">选择资源</option>{resources.filter((resource) => resource.lifecycle.state === "published").map((resource) => <option key={resource.id} value={resource.id}>{resource.title}</option>)}</select></label><label className="grid gap-1 text-sm text-slate-700">更正上一版本<select className="h-10 rounded-md border border-slate-300 bg-white px-3" value={questionForm.previous_version_id ?? ""} onChange={(event) => setQuestionForm({ ...questionForm, previous_version_id: event.target.value || null })}><option value="">新增首版</option>{questions.filter((question) => question.lifecycle.state === "published").map((question) => <option key={question.id} value={question.id}>{question.prompt} v{question.version}</option>)}</select></label><label className="grid gap-1 text-sm text-slate-700">正确选项编号<Input aria-label="正确选项编号" value={questionForm.correct_option_id} onChange={(event) => setQuestionForm({ ...questionForm, correct_option_id: event.target.value })} required /></label>
