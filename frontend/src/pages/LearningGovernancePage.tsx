...
function errorMessage(cause: unknown) {
  return cause instanceof ApiClientError ? cause.message : "暂时无法完成内容治理操作，请稍后重试。";
}
...
function LifecycleActions({
  ...
  return action ? <Button size="sm" variant={action === "withdraw" ? "danger" : "secondary"} isDisabled={busy} onPress={() => onAction(action)}>{labels[action]}</Button> : null;
}
...
export function LearningGovernancePage() {
  ...
  const actOnResource = (resource: ManagedLearningResource, action: "deidentify" | "review" | "publish" | "withdraw") => {
    if (!token || !reason.trim()) { setError("请填写本次操作理由。"); return; }
    setBusyId(resource.id);
    void transitionManagedLearningResource(token, resource.id, action, reason).then(() => { setReason(""); return load(); }).catch((cause) => setError(errorMessage(cause))).finally(() => setBusyId(""));
  };
  const actOnQuestion = (question: ManagedLearningQuestion, action: "deidentify" | "review" | "publish" | "withdraw") => {
    if (!token || !reason.trim()) { setError("请填写本次操作理由。"); return; }
    setBusyId(question.id);
    void transitionManagedLearningQuestion(token, question.id, action, reason).then(() => { setReason(""); return load(); }).catch((cause) => setError(errorMessage(cause))).finally(() => setBusyId(""));
  };
  const submitResource = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!token) return;
    setBusyId("new-resource");
    void createManagedLearningResource(token, { ...resourceForm, tags: parseTags(resourceTags), source_url: resourceForm.source_url?.trim() || null }).then(() => { setResourceForm({ ...resourceForm, title: "", summary: "", content: "", tags: [], source_name: "", source_url: null, submission_reason: "" }); setResourceTags(""); return load(); }).catch((cause) => setError(errorMessage(cause))).finally(() => setBusyId(""));
  };
  const submitQuestion = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    void createManagedLearningQuestion(token, { ...questionForm, tags: parseTags(questionTags), options }).then(() => { setQuestionForm({ ...questionForm, prompt: "", tags: [], options: [], correct_option_id: "", explanation: "", submission_reason: "" }); setQuestionTags(""); setOptionLines(""); return load(); }).catch((cause) => setError(errorMessage(cause))).finally(() => setBusyId(""));
  };
