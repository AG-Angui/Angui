  ...
  return <main className="mx-auto w-full max-w-5xl px-4 py-7 sm:px-6 lg:px-10 lg:py-10">
    ...
    {loading ? <LoadingState label="正在加载分配给你的任务" /> : tasks.length === 0 ? <EmptyState icon={Compass} title="当前没有分配给你的任务" description="任务被指挥分配并授权后，会显示在这里。" /> : <section className="grid gap-4" aria-label="我的任务列表">{tasks.map((task) => {
      ...
      return <article key={task.id} className="rounded-md border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="m-0 text-base font-bold text-slate-950">{task.title}</h2><p className="mb-0 mt-1 text-sm text-slate-700">{task.objective}</p></div><Chip size="sm" variant="soft"><Chip.Label>{statusLabels[task.status]}</Chip.Label></Chip></div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">{statusActions(task.status).map((status) => <Button key={status} size="sm" variant={status === 'completed' ? 'primary' : 'secondary'} isDisabled={pendingTaskIds.has(task.id)} onPress={() => void changeStatus(task, status)}>{statusLabels[status]}</Button>)}<Button size="sm" variant="ghost" isDisabled={pendingTaskIds.has(task.id)} onPress={() => { if (!token) return; void run(task.id, async () => { const result = await getTaskNavigation(token, task.id); setNavigation((current) => ({ ...current, [task.id]: result })) }, '已加载任务导航说明。') }}><Compass size={16} />导航说明</Button><Button size="sm" variant="ghost" isDisabled={pendingTaskIds.has(task.id)} onPress={() => { if (!token) return; void run(task.id, async () => { const result = await getTaskSafetyBriefing(token, task.id); setSafety((current) => ({ ...current, [task.id]: result })) }, '已加载安全提示。') }}><ShieldCheck size={16} />安全提示</Button></div>
        ...
        {safety[task.id] && <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-slate-700"><div className="flex items-center gap-2 font-semibold text-amber-900"><AlertTriangle size={16} />任务安全提示</div><ul className="mb-0 mt-2 list-disc space-y-1 pl-5">{safety[task.id].notices.map((item, index) => <li key={`${task.id}-${index}`}>{item}</li>)}</ul><p className="mb-0 mt-2 text-xs text-amber-900">{safety[task.id].emergency_stop_message}</p></div>}
        {active && <div className="mt-4 grid gap-4 border-t border-slate-200 pt-4 lg:grid-cols-2"><form className="grid gap-2" onSubmit={(event) => { event.preventDefault(); if (!token) return; if (!taskLocation.latitude.trim() || !taskLocation.longitude.trim() || !taskLocation.accuracy.trim()) { setError('请填写模拟纬度、经度和定位精度。'); return } const latitude = Number(taskLocation.latitude); const longitude = Number(taskLocation.longitude); const accuracy = Number(taskLocation.accuracy); if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90 || !Number.isFinite(longitude) || longitude < -180 || longitude > 180 || !Number.isFinite(accuracy) || accuracy <= 0) { setError('请填写有效的模拟经纬度和大于 0 的定位精度。'); return } void run(task.id, async () => { await submitTaskLocationReport(token, task.id, { source: 'simulated_demo', latitude, longitude, accuracy_meters: accuracy, captured_at: localNow() }) }, '模拟位置已上报；仅用于本任务演示，可随时停止上报。') }}><h3 className="m-0 text-sm font-semibold text-slate-950"><LocateFixed className="mr-1 inline" size={16} />模拟位置上报</h3><p className="m-0 text-xs text-slate-600">这是手动模拟演示，不请求浏览器后台定位；任务暂停、完成或取消后不可上报。</p><div className="grid grid-cols-3 gap-2"><Input aria-label="模拟纬度" type="number" value={taskLocation.latitude} onChange={(event) => setLocation((current) => ({ ...current, [task.id]: { ...taskLocation, latitude: event.target.value } }))} placeholder="纬度" /><Input aria-label="模拟经度" type="number" value={taskLocation.longitude} onChange={(event) => setLocation((current) => ({ ...current, [task.id]: { ...taskLocation, longitude: event.target.value } }))} placeholder="经度" /><Input aria-label="定位精度米" type="number" value={taskLocation.accuracy} onChange={(event) => setLocation((current) => ({ ...current, [task.id]: { ...taskLocation, accuracy: event.target.value } }))} placeholder="精度(米)" /></div><Button type="submit" size="sm" variant="secondary" isDisabled={pendingTaskIds.has(task.id)}>上报模拟位置</Button></form><form className="grid gap-2" onSubmit={(event) => { event.preventDefault(); if (!token || !feedback[task.id]?.trim()) return; void run(task.id, async () => { await submitTaskFeedback(token, task.id, { content: feedback[task.id].trim(), occurred_at: localNow(), location_text: task.area_text, location_precision: 'approximate' }); setFeedback((current) => ({ ...current, [task.id]: '' })) }, '执行反馈已作为待审核线索提交。') }}><h3 className="m-0 text-sm font-semibold text-slate-950">执行反馈</h3><p className="m-0 text-xs text-slate-600">反馈会进入人工审核，不会自动成为确认事实。</p><TextArea aria-label="执行反馈内容" value={feedback[task.id] ?? ''} rows={3} maxLength={4000} onChange={(event) => setFeedback((current) => ({ ...current, [task.id]: event.target.value }))} fullWidth /><Button type="submit" size="sm" variant="secondary" isDisabled={pendingTaskIds.has(task.id) || !feedback[task.id]?.trim()}>提交反馈</Button></form></div>}
      </article>
    })}</section>}
  </main>
}
