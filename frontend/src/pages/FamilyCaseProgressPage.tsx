...
export function FamilyCaseProgressPage() {
  ...
  const [place, setPlace] = useState<CreateCasePlacePayload>({ name: "", place_type: "frequent", address: "", longitude: null, latitude: null, visibility: "confirmed" });
  ...
  async function submitPlace(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      await createCasePlace(token, caseId, { ...place, name: place.name.trim(), address: place.address.trim() });
      setPlace({ name: "", place_type: resources.case_place_types[0] ?? "frequent", address: "", longitude: null, latitude: null, visibility: "confirmed" });
