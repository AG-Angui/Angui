...
export function submitTaskLocationReport(token: string, taskId: string, payload: { source: 'simulated' | 'simulated_demo'; latitude: number; longitude: number; accuracy_meters: number; captured_at: string }, idempotencyKey = crypto.randomUUID()): Promise<TaskLocationReportReceipt> {
  return apiRequest<TaskLocationReportReceipt>(`/tasks/${taskId}/location-reports`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ ...payload, source: 'simulated' }) }, token)
}
