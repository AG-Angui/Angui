...
- `users.role` 是固定的全局平台身份（`family`、`commander`、`volunteer`、`learner`、`admin`），`case_memberships.role` 是固定的案件内身份（`family`、`commander`、`volunteer`）。两者均以跨 SQLite、PostgreSQL、MySQL 一致的 `CHECK` 约束保存；业务层使用 Rust 枚举转换未知值，未知持久化值按数据完整性错误处理，绝不默认授权。
