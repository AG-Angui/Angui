...
The current creation API writes only `collecting`. The answer API advances a session with complete required answers to `ready_for_confirmation`; the confirmation API atomically creates the case, elder profile, family membership, and `case.created` audit event before setting `case_id`, `confirmed_by_user_id`, `confirmed_at`, and `status=confirmed`. Sensitive raw answers remain in the session record and are deliberately excluded from audit metadata and ordinary logs. The database limit is intersected with the server-only `ANGUI_INTAKE_ANSWER_HARD_MAX` safety cap; database configuration cannot increase that hard limit.

`ai_prompt_templates` is the platform-level, versioned store for approved AI system instructions. It records the purpose, immutable version, instruction body, lifecycle status, and creation/publication attribution. `purpose` is constrained to the currently defined business purposes rather than having separate prompt tables per module. Runtime business records persist only the selected template version; they never persist a raw prompt in audit metadata. The table is seeded with the `intake_next_question` template for future model-backed guidance, while the current intake endpoint remains rule-based. A later admin configuration API must create, publish, and retire versions with audit events; an intake request can never supply its own prompt.
